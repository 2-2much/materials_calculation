...
**검증 상태(미완):** 파이프라인은 기계적으로 정상·자기일관적이나 물리적 정확성은 아직 미확정. 보정의 본질=「보정된 E_f가 셀 크기에 무관」이므로 검증법: ① ⭐lateral 셀 크기 수렴(3×2 vs 더 큰 셀 → 보정된 E_f 일치해야) ② slabcc/SCPC 교차검증 ③ charge center z0=0.80/0.81/0.82 민감도 ④ potential_profile.pdf의 diff(FP−Gauss)가 결함 먼 영역에서 평탄한지 ⑤ 근사 ε 대신 LEPSILON. 사용자가 NK/Komsa 방법론(FNV 확장: 모델 가우시안 전하를 ε(z)에 넣고 푸아송, E_corr=(E_iso−E_per)−qΔV) 학습 중. 참고: Komsa PRX 4,031044(2014); Noh et al PRB 89,205417(2014); FNV PRL 102,016402(2009).

...
- **Ecut은 크게 과잉이었다**(2026-08-03, MoS₂ α=20): 예제 기본 15 Ha 대비 **8 Ha가 0.6 meV·2배 빠름, 6 Ha가 0.8 meV·3.8배, 4 Ha가 1.1 meV·12.9배**. 작업량 ∝ Ecut^1.5. Ecut을 요구하는 건 가우시안이 아니라 **유전 계면**(σ=1.89 bohr 가우시안은 Ecut 15에서 exp(-G²σ²/2)=5e-24로 이미 무의미, Smoothness=0.378 bohr erf는 0.34로 살아있음). ⚠단 E_per 한 점이 아니라 **E_iso 외삽이 안 흔들리는지**를 확인해야 채택 가능(2단계 검증).
- ✅**InAs(In_As_1)에서도 Ecut 20→8 통과**(2026-08-03): ①스윕은 Ecut 4~30이 **산포 4.5 meV·비단조 = 이미 잡음 수준**(σ=2.6205로 MoS₂보다 넓어 더 관대). ②전체 스캔에서 α별 차이가 −0.4~−2.1 meV로 **부호·크기 균일** → E_iso 0.4339→0.4354(+1.5 meV), **E_lat 최대 편차 3.1 meV**(MoS₂의 0.00 meV보다는 크나 외삽 불확실도 ±10~25 meV의 1/5, 전 케이스 균일 2.3~3.1 meV라 비교엔 무영향). 비용 α=8이 54→12초, 13개 세트 90→21초. **권고: 신규 계산만 Ecut 8, 기존 프로덕션 값은 20 그대로 둘 것**(3 meV 위해 갈아엎을 이유 없음).

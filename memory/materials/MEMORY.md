...
- [Shallow-limit DFE Construction](shallow_limit_dfe_construction.md) — shallow donor DFE 작도 **E_f(+1,E_F)=E_f(0)+(E_F−E_g)** 채택(2026-07-17). 하전상태 total energy를 안 써서 charged correction을 통째 우회. 조건=bound로 표기(점선)/E_f(0)에 band-filling 선행/V_Cl-Cl_As에 +2선 금지. ⚠band-filling(~0.78eV, 형성E와 맞먹음)이 파이프라인에 **없음**=진짜 구멍. Γ-only는 이걸 0으로 오판→4×4×1 이상 또는 lateral 셀 확대 필요

...
- [slabcc Delocalized Defect Policy](slabcc_delocalized_defect_policy.md) — ⚠delocalized 결함 slabcc 실패=버그 아닌 물리(PHS엔 model-charge 보정이 범주 오류, 무한시트 E_iso 발산). 판정=IPR/E_relax/optimize_charge_position=yes 게이트. Cl-As_In q+1만 신뢰, As_In·V_Cl-Cl_As 폐기→shallow 재분류. ⚠plot_DFE가 실패 E_corr을 조용히 0으로 대체→CTL_summary에 가짜 0.99eV 등재(미수정)

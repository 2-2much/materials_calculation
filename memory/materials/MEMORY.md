...
- [Defect Package Repo](defect_package_repo.md) — Defect_Package git repo=/mnt/hohenberg/.../scripts/Defect_Package(로컬only, 원격없음). 계산폴더는 clone아닌 복사본→수동동기화. scripts 3개 커밋(2026-07-06), surface config 미반영. +운용모델(2026-07-14): 개발은 정본repo에서 직접commit, 계산폴더는 origin제거(pull merge회피)+scripts symlink/cp 반영. +커밋컨벤션(2026-07-14): 로컬커밋만/push안함, 영어명령형제목+원자적커밋, git -C 사용, Co-Authored-By 여부 미결. 다음=/clear후 플랜모드 패키지 업그레이드. +사고(2026-07-15, 진짜원인): calc `scripts`가 /mnt/hohenberg(NFS)로의 심링크라 compute노드서 dangling→seed 즉사→afterok 데드락. SLURM경로엔 symlink금지, 실복사(rm scripts&&cp -r) 필요. 내일 처리예정

...
`In_i_sub`는 mag 0.72/EENTRO −0.032로 정수 점유 미달이었고 **NUPDOWN=1로 확정됨(아래)**.
⚠ ΔE 잡음 바닥은 **±0.3 meV**(00은 EDIFF=1E-5, 01은 1E-4) — 양수 0.2 meV 따위는 무시.

## ✅ NUPDOWN=1 검증 완료 (2026-08-10, 잡 55988~55992) — IPR 규칙이 또 맞았다
대칭해(occ≈0.5)에 갇힌 5종을 `NUPDOWN=1`로 재이완. **ΔE = E₀(NUPDOWN) − E₀(자유스핀), σ→0 기준.**

| defect | IPR | ΔE | mag | 판정 |
|---|---|---|---|---|
| **In_i_sub** | 7.67× | **−9.85 meV** | 1.001 | ★ **자성 확정** |
| Cl_i-In | 2.97× | −1.46 meV | 1.001 | 축퇴(판정 불가) |
| V_As | 2.75× | −0.40 meV | 1.000 | 축퇴(판정 불가) |
| V_In | 1.44× | **+12.01 meV** | 1.001 | **비자성 확정** |
| Cl-As_In | 1.89× | **+14.46 meV** | 1.000 | **비자성 확정** |

- ⚠ **`In_i_sub`의 q0 에너지를 갱신할 것: E₀ = −449.77630** (자유스핀 −449.76645는 mag 0.72에
  갇힌 미수렴 해). 비자성 대비 제대로 된 ΔE(spin) = **−24.9 meV**(기존 보고 −15.1 meV는 폐기).
- **회색지대(1.2~2×)에서 NUPDOWN=1이 12~14 meV 손해** → 진짜 비자성. IPR>6×에서는 이득.
  [[spin_magnetism_ipr_predictor]]의 규칙이 예외 없이 재현됐다.
- IPR 2~3×인데 ΔE가 잡음 바닥(±0.3 meV)인 `V_As`·`Cl_i-In`은 **속박이나 교환분열≈0**.
  어느 쪽으로도 확정 불가이고 에너지 차가 없어 DFE에는 무영향. "자성 결함"으로 서술 금지.
⇒ **이 세트의 자성은 `V_Cl`(mag 0.974, ΔE −54 meV)과 `In_i_sub` 둘뿐.**

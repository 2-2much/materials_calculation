---
name: scpc-debug
description: "VASP SCPC getgrid() 버그 해결, CKT 호환성, SCF 발산 문제 진행 중"
metadata: 
  node_type: memory
  type: project
...
경로: `33-inAs/__Functional_Validation__/11-Surface-defect_TOY-model/00_TOY_DEBUG/2x2x1/02_vacuum_scan/CKT/new_c_40A/As_In/q-1/03-SCPC/`
...
### CKT + SCPC 일관성
REFPOT/REFCHG(q0)와 WAVECAR(q-1 1shot) 모두 CKT 사용. SCPC INCAR에도 `KERNEL_TRUNCATION` 블록 추가 완료. scpc.F의 `scpc_get_vhar`가 표준 `pothar` 호출하므로 호환됨.

### SCF 발산 문제 (미해결, 2026-06-22)
CKT + SCPC + MGX/MGY/MGZ 적용 후 SCF 발산 확인:
- DAV step 12에서 에너지 -254 → -495 eV 급락, rms 0.15 → 1.55
- crash 아님, scancel로 수동 취소 (job 54820)
- SCPC correction이 매 step 적용되면서 mixing 불안정 가능성

다음 시도 후보:
1. ALGO=All (Davidson + RMM-DIIS)
2. AMIX/BMIX 줄이기
3. SCPC/IN 값 증가 (4~5, 초기 step에서 SCPC 없이 수렴 후 적용)

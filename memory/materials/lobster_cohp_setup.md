...
description: "02-Cl-passv 슬랩 COHP/LOBSTER 셋업 — 최소 524밴드, In_d는 4d 기저 포함. ⚠LREAL=.FALSE.는 '매뉴얼 요구'가 아니라 권장(2026-08-11 정정)"
metadata: 
  node_type: memory
  type: project
...
주의점:
- **LREAL=.FALSE. 권장 — 단 "매뉴얼이 요구한다"는 서술은 틀렸다(2026-08-11 정정).** LOBSTER 5.1.0 `Lobster_Users_Guide`/`FAQ` 어디에도 LREAL 요구사항이 없고, 번들 VASP 예제 5개 중 **carbon-nanotube와 fullerene은 `LREAL = Auto`**로 되어 있다(diamond/gaas/ti만 `.FALSE.`). 즉 LOBSTER가 금지하지 않는다. 남는 것은 아래 물리 논거뿐이며, **실제 판정은 charge spilling 실측으로 할 것**(A/B 테스트: `14-PBE_COHP_VAs-vs-ClAs/B2_surf_Cl_As/{01_DOS-COHP,__LREAL-A_test__}`). VASP는 근사 projector로 수렴시킨 ψ̃를 WAVECAR에 쓰는데 LOBSTER는 POTCAR의 정확한 역격자 projector로 PAW 복원을 다시 함 → 해밀토니안 불일치가 charge spilling 폭증으로 나타남. 에너지/힘은 변분 보호로 멀쩡하지만 파동함수 자체를 소비하는 LOBSTER는 직격당함.

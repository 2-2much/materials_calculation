...
| `6.5.0/vasp.6.5.0.**gam**.mpi.x` | ✗ illegal instruction ← 2026-08-25 (22번 트리 17잡 전멸) |
| `6.3.2/vasp.6.3.2.**gam**.x` | **OK** ← 2026-08-25 확정 |

→ **`*.mpi.x` 계열이 이 CPU에 안 맞게 빌드돼 있다.** 사용자 확인: VASP는 서버·노드마다 따로 컴파일해 두었고 표준 빌드는 여기서 잘 돈다.
★ **gam 변형도 규칙이 같다** (2026-08-25 실측): `6.5.0/…gam.mpi.x` 는 std 와 똑같이 죽고,
`6.3.2/vasp.6.3.2.gam.x` 는 정상이다 — 33번 트리 pure/q0 이 229 이온스텝 완주(2026-08-19),
22번 트리 17잡도 이 바이너리로 정상 기동. **sham 에서 Γ-only 단계는 `6.3.2/vasp.6.3.2.gam.x`.**

⚠ **How to apply — 이 문서를 *고르기 전에* 읽어라.** 2026-08-25 에 나는 "kohn 의 6.5.1 과
가까운 버전"이라는 이유로 `6.5.0/…gam.mpi.x` 를 골랐고 17잡이 2초 만에 전멸했다. 이 표에
`6.5.0/…std.mpi.x ✗` 와 "`*.mpi.x` 계열은 이 CPU 에 안 맞다" 가 **이미 적혀 있었다**.
버전 근접성이 아니라 **파일명(빌드 변형)** 으로 고를 것. 같은 서버의 최근 트리
(sham 이면 33-100MACl…) 의 `runtime.yaml` 을 그대로 복사하는 것이 가장 안전하다.
...
## ★2026-09-07 g2 실측 추가 — `6.5.1/…lhfskip.gam.x` 정상 + std 와 에너지 일치
`vasp.6.5.1.dftd4.wan90.beef.plugin.lhfskip.gam.x` 를 **g2 에서 64잡 완주**(hBN 72~162원자, Γ-only).
OUTCAR 헤더 `gamma-only` 확인. **std 로 돌린 07 트리와 대조: JCC 형성에너지가 ≤6 meV 일치**
(4.909/4.994/5.040/5.078 vs 4.903/4.989/5.035/5.073) → **gam 변형 검증 완료, 별도 Si 테스트 불필요**.
표의 규칙(`*.mpi.x` ✗ / lhfskip 빌드 ✅)이 gam 에도 그대로 적용됨을 재확인.
⚠ 여전히 [[surface_defect_istart_wavecar_gam_std]]: gam WAVECAR 을 std 가 못 읽는다.
단일점 트리는 `LWAVE=.FALSE.` 로 두면 이 문제가 원천 소멸(디스크도 9 GB 절약).

...
★ **trench 지붕을 막는 것은 메틸이 아니라 두 MA 의 아민 H** 다 (H166=N130/In22, H159=N129/In21).
2026-08-19 사용자 요청으로 `In_i_trench-Cl` 의 Cl 을 옆이 아니라 **In_i 바로 위(+z, 2.500 Å)** 로
옮겼는데, 그냥 놓으면 아민 H 와 **1.80 Å** 충돌 → 그 두 MA 를 각자의 **In–N 축 둘레로 +36°/+48°
강체 회전**시켜 지붕을 열었다(Cl···H 2.17/2.21 Å). 강체 회전이라 MA 내부 결합은 pure 와 완전 동일.
⚠ 이때 목적함수에서 **결합(In–N 2.394, In_i–As 2.464)을 빼지 않으면** 최적화가 그 결합값에
포화돼 정작 Cl···H 가 안 좋아진다 — 두 번 헛돌았다.

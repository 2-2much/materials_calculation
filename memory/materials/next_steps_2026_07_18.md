...
**⚠HSE Tier-2 수렴 교훈(2026-07-20)**: (1) VASP lhfskip 바이너리는 **수정된 QE식**(HSE06 1shot→PBE 재수렴 반복)이라 **dE 급증은 정상**(charge sloshing 아님)—**rms(c) 감소**를 볼 것. (2) **ALGO=Damped는 ISTART=0(scratch)서 발산**(E→−62000eV, 크래시). 필요시 **ALGO=Normal + ICHARG=1(00_Gam-relax CHGCAR seed)** 사용(=02_G221-DOS 검증 레시피). 1×1×1 defect HSE 완료 −413.880(참고용, calc/__k-point_test__/Vcl_neutral_HSE/).
**⚠결정(2026-07-20): HSE k-scan 전면 취소·불필요.** Γ-only가 band-filling을 우회(전자가 CBM에 앉아 bf=0)하고 DFE는 PBE-d로 검증한 ~0.13eV 이내 수렴이며 n형 주장은 정성적(E_f 뚜렷이 음수)이라 결론 불변. **DFE는 Γ-only(00_Gam-relax)로 확정**(핵심결함 포함). DOS/band용 SCF k-grid는 **PBE로 별도 수렴테스트(내일, 2026-07-21 예정)** — grid밀도는 함수 무관하고 shallow 결함준위는 k에 둔감. HSE k-scan 잡 55446-55451/55488-55496 전부 취소.

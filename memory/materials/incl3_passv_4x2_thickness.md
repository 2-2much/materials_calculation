...
**Step0 실측(2026-07-07, 기존 2×1 6L LOCPOT=6×6×1/dipoleOFF/LVHAR):** 내부 macro-avg(4.36Å창) pk-pk: 전체내부(z10.5-17.5)=331meV, 하부(10.5-15)=86meV, 가장평평bilayer(11.3-13.5)=**27meV**. ⚠**선형제거해도 331→284meV**(slope<30meV/Å)→tilt는 제거가능 dipole ramp 아니라 **진짜 비선형=InCl3 표면섭동이 중앙까지 침투**→**dipole 보정 켜도 소용없음**(dielectric-ramp 가설 데이터로 기각). 섭동은 위(InCl3)쪽 지배, 아래(pseudo-H+고정층)가 더 bulk-like. 진공엔 ~4.8eV dipole sawtooth(내부무관). **판정 두 기준**: ①넓은plateau→6L부족(평평한건 z≈12 1bilayer 20meV뿐, 5L탈락, 이기준이면 7-8L). ②수렴된 bulk 기준면→정렬엔 넓은plateau 불필요, bulk값 수렴한 기준면 1곳(z≈12 flat-spot≈−4.73eV LVHAR)이면 충분→6L도 살수있음. **다음 최소액션: 7L 하나만 같은프로토콜로 돌려 flat-spot 절대값 수렴 확인**(6L와 ≲20-30meV 일치→6L flat-spot을 정렬기준면 채택, charged도 신뢰/어긋나면 8L). 중성 E_f는 지금 신뢰. macro.py에 planar+macro+선형fit잔차 기능 있음.

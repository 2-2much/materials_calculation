...
## ★ bare vs pseudo-H 직접 대조 (2026-08-19, macroavg.py track-A)

`macroavg.py`는 슬랩 고유값을 안 쓰므로 **금속인 bare 극성면에도 적용 가능**하다. 이걸로 갈랐다:

| 면 | bare (H 없음) | + pseudo-H | Δ |
|---|---|---|---|
| (100) In-종단 | **4.42** | 5.48 | **+1.05** |
| (111) In-종단 | **3.69~3.95** | 5.43 | **+1.5~1.7** |
| (110) 혼합 | 4.78 | 4.86 | +0.08 |

**bare 서열 (110) 4.78 > (100) 4.42 > (111) 3.7 — pseudo-H 서열과 완전히 뒤집힘.**

→ **"In 종단이라서 표면 양전하 → IP 높다"는 틀렸다. In 종단 자체는 IP를 오히려 낮춘다**
(노출된 In dangling bond 밴드가 E_F를 걸치는 저일함수 금속면). 0.6 eV를 올린 건 **전적으로 H1.25 층**.
맞는 부분은 쌍극자 방향뿐: In^δ+ ⋯ H^δ−(바깥이 음) → 진공준위↑ → VBM이 진공 대비 깊어짐.

⚠ bare 극성면 주의: 금속이라 이 IP는 정전기적으로만 정의됨(실제 광전자 문턱 아님).
(111) bare는 두께 미수렴(3.951→3.687)에 내부 ripple 177~259 meV → ±0.15 eV로 볼 것.
(100) bare는 4.456→4.423 수렴, ripple 50~95 meV로 신뢰 가능.
